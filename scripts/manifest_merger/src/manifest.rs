...
impl Manifest {
    ...
    pub fn get_url(&self) -> Option<String> {
        self.tag.as_ref().map(|tag| {
            format!(
                "https://git.codelinaro.org/clo/la/la/{0}/manifest/-/raw/release/{1}.xml",
                self.name, tag
            )
        })
    }
    ...
    pub fn get_tag(&self) -> Option<String> {
        self.tag.as_ref().map(|tag| format!("{tag}"))
    }
...
pub fn update_default(
    ...
    if system_manifest.as_ref().is_some() {
        let msg = format!(
            "system: Update to {}",
            system_manifest.as_ref().unwrap().get_tag().unwrap()
        );
        ...
        let msg = format!(
            "vendor: Update to {}",
            vendor_manifest.as_ref().unwrap().get_tag().unwrap()
        );
        git::add_and_commit(&repo, ".", &msg)
            .map_err(|err| format!("Failed to commit version change: {err}"))?;
    }
