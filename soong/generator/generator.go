...
func init() {
	android.RegisterModuleType("zephyrus_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zephyrusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zephyrusExpandVariables(ctx, dep_file)
	...
	cmd := zephyrusExpandVariables(ctx, String(g.properties.Cmd))
